...
struct Data *find(struct Data *curr, int value) // recursion
{
	...
	// x is found
		...
		return find(curr->left, value); // val is located in left sub tree
	else
		return find(curr->right, value); // val is located in right sub tree
}
...
void searchDisplay(struct Data *root, int value)
{
	...
	if (curr)
	{
		...
		if (curr == root)
		{
			...
			printf("- is an external node(leaf)\n");
		else
			printf("- is an internal node\n");
